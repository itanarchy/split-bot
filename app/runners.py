...
async def webhook_startup(dispatcher: Dispatcher, bot: Bot, config: AppConfig) -> None:
    ...
    loggers.webhook.info("Trying to set main bot webhook on url %s", url)
    if await bot.set_webhook(
        ...
        return loggers.webhook.info("Main bot webhook successfully set")
    return loggers.webhook.error("Failed to set main bot webhook")
